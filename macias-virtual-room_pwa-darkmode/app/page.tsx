...
function ThemeToggle(){
  ...
  return <button className="btn btn-outline" onClick={()=>setDark(v=>!v)}>{dark? "Light" : "Dark"} mode</button>
}
...
const mockPlans: PlanFile[] = [
  ...
  { id: "p-003", title: "SH 45 Toll Ramps – Plan Set", district: "Austin", csj: "0685-01-067", highway: "SH 45", letDate: "2025-09-11", version: "IFB v2", size: "73 MB", url: "#", tags: ["Drainage","Pavement"] },
  { id: "p-004", title: "FM 1960 Intersection Improvements", district: "Beaumont", csj: "1685-05-023", highway: "FM 1960", letDate: "2025-07-31", version: "Final", size: "34 MB", url: "#", tags: ["Traffic Control","Utility"] },
]

const uid = () => Math.random().toString(36).slice(2) + Date.now().toString(36)

function saveSession(user: User | null) {
  if (user) localStorage.setItem("vr_session_user", JSON.stringify(user))
  else localStorage.removeItem("vr_session_user")
}
function getSession(): User | null {
  const raw = localStorage.getItem("vr_session_user")
  return raw ? JSON.parse(raw) as User : null
}
function mockAuth(email: string): User {
  return { id: uid(), email, role: email.endsWith("@maciasspecialty.com") ? "admin":"subcontractor", name: email.split("@")[0] }
}
    ...
    <header className="sticky top-0 z-20 bg-white/70 backdrop-blur border-b">
      <div className="container py-3 flex items-center gap-4">
        ...
        <nav className="ml-auto hidden sm:flex items-center gap-6 text-sm">
          ...
          {onSignIn && <button className="btn btn-primary" onClick={onSignIn}><LogIn className="w-4 h-4"/> Sign in</button>} <ThemeToggle />
        </nav>
      </div>
    </header>
  )
...
function LandingPage({ onStart }: { onStart: () => void }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-white to-macias-light/20">
      ...
      <section className="container py-16 grid md:grid-cols-2 gap-10 items-center">
        <div>
          ...
          <p className="mt-4 text-neutral-600 text-lg">A modern, secure room where subcontractors can view and download project plan sets released by TxDOT.</p>
          ...
          <div className="mt-6 text-xs text-neutral-500">Access requires a verified work email.</div>
        </div>
        <div className="card">
          <div className="card-h text-sm font-medium">Preview</div>
          <div className="card-c grid sm:grid-cols-2 gap-3">
            {mockPlans.map(p => (
              <div key={p.id} className="card border-macias/30">
                <div className="card-c pb-0">
                  ...
                  <div className="text-xs text-neutral-500 mt-1">{p.district} · {p.highway}</div>
                </div>
      ...
      <section className="container py-16 text-center">
        ...
        <p className="text-neutral-600 mt-2">Use your company email to sign in.</p>
...
function SignIn({ onAuthed }: { onAuthed: (user: User) => void }){
  const [email, setEmail] = useState("")
  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    const u = mockAuth(email.trim().toLowerCase())
    saveSession(u); onAuthed(u)
  }
  return (
    <div className="min-h-screen grid place-items-center bg-gradient-to-br from-macias-light to-white">
      <div className="card w-full max-w-md border-macias">
        <div className="card-c flex items-center flex-col gap-2">
          ...
          <div className="text-sm text-neutral-600">Sign in to view TxDOT plan sets</div>
        </div>
...
function Sidebar({ user, onSignOut }:{ user: User, onSignOut:()=>void }){
  return (
    <aside className="border-r w-64 h-full bg-neutral-50 flex flex-col">
      <div className="card-h flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-xs text-neutral-500">Subcontractor Portal</div>
        </div>
      </div>
      <nav className="p-2 space-y-1">
        <button className="w-full text-left px-3 py-2 rounded-xl hover:bg-neutral-100 flex items-center gap-2"><Building2 className="w-4 h-4"/> Districts</button>
        <button className="w-full text-left px-3 py-2 rounded-xl hover:bg-neutral-100 flex items-center gap-2"><Map className="w-4 h-4"/> Highways</button>
        <button className="w-full text-left px-3 py-2 rounded-xl hover:bg-neutral-100 flex items-center gap-2"><Users className="w-4 h-4"/> Members</button>
      </nav>
